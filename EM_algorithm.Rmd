...
We can see in the following graph that many times the 3 clusters overlap.
...
plot_ellipses_CI(X,K,y_text) +
  xlab(colnames(X)[1]) +
  ylab(colnames(X)[2]) +
  ggtitle('Fixed Acidity and Alcohol of the wines grouped by the true clusters')
...
We extract different number of clusters (from 2 to 5 clusters).
...
Therefore, higher ARI indicates better results.
On the other hand, lower Classification error (CE) implies better results.
...
Just by observing the ARI and CE we cannot establish a winner between the two types of initialization. 
Nonetheless, by observing the groupings, it is clear that the option with three clusters and random centroids initialization is the one that emulates the most the truth groups. 
...
#WIP: add interpretation to the results of the means, add titles with number of clusters, initialization. axis y should say "Alcohol" or "Fixed Acidity".
...
#WIP: add interpretations on loglikelihood
