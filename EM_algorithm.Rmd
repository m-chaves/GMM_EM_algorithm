...
* Given a dataset $X$ and a number of desired clusters $K$, `EM_algorithm` computes the EM algorithm. One of the previously described initializations must be chosen. Also, knowing that the EM-algorithm is sensible to the initialization of the parameters. That is, different initializations can lead to different local maxima of the log-likelihood, the parameter 'number of initialization' defines the number of times to run the EM-algorithm with different initializations. The function outputs the results related to the best maximum found. 
...
* `crossvalidation.EM` performs k-fold cross-validation on the EM-algorithm.
* 
...
# WIP: axis names, titles for the graphs (indicating number of clusters and initialization), make nicer tooltip of the error (if possible) and report also the result of adjustedRandIndex. 
