...
EM_algorithm = function(X,K,max_it=1000, initialization, number_of_initialization = 10){
  ...
  X = as.matrix(X)
  n = dim(X)[1] #number of observations
  mu_hist <- matrix(ncol=K*2, nrow=max_it)
...
#drawing
mu_df = data.frame(mu)
p <- ggplot(data=df, aes(x=x1, y=x2,colour=group)) + geom_point(size=1.5, alpha=.6) + geom_path(data=df_ell, aes(x=x1, y=x2,colour=group), size=1, linetype=2) +geom_point(data=mu_df,aes(x=mu_df[,1],y=mu_df[,2]), shape=4, color='black',size=3) + theme_bw()
...
#drawing
p <- ggplot(data=df, aes(x=x, y=y,colour=group)) + geom_point(size=1.5, alpha=.6) + geom_path(data=df_ell, aes(x=x, y=y,colour=group), size=1, linetype=2) + theme_bw()
...
plot_loglik_by_cluster = function(validation_loglik,num_clusters){
  ...
  p = df %>% 
    ...
    geom_boxplot() +
    geom_point(data = df_mean, aes(`Number of clusters`, y=`Mean`, colour = 'Mean'), show.legend = TRUE) +
    scale_color_manual(values = c("Mean" = 'brown2')) +
    labs(color = "") +
    theme_bw()
  return(p)
}
...
WIP: Add some description of this graph (the one with ellipses) 
...
annotate_figure(plt_final, top = text_grob("Clustering with GMM using EM algorithm in wine data where: x1=Alcohol x2=Fixed Acidity ", color = "red", face = "bold", size = 16), left = text_grob("Metrics to evaluate the clustering ARI = Adjusted Rand Index, CE = Class error", color = "blue", face = "bold", size = 14, rot=90))

```


```{r mu plots, fig.height=18, fig.width=12}
...
plots_list = append(plots_list,plot_hist_mu(data_mu, K))
...
ggarrange(plotlist = plots_list, nrow = 4, ncol = 4,legend = "none")
```

```{r loglik plots, fig.height=18, fig.width=12}
plots_list = list()
for (plt_n in c(1:length(loglik_list))){
  loglik_hist = loglik_list[[plt_n]]
  p1 = plot_hist_loglik(loglik_hist)
  plots_list[[plt_n]] = p1
}

ggarrange(plotlist = plots_list, nrow = 4, ncol = 2,legend = "none")

# plot_list_loglik = function(loglik_list){
# 
# 
# data_loglik <- matrix(ncol=length(loglik_list), nrow=1000)
# 
# plots_list = c()
# for (plt_n in c(1:length(loglik_list))){
# 
# data_loglik[,1] <- loglik_list[[1]]
# data_loglik[,1] <- loglik_list[[1]]
# loglik_list[[1]]
# plots_list = append(plots_list,plot_hist_mu(data_mu, K))
# 
...
plot_AIC_BIC_by_cluster(aic_vector, bic_vector, min_clusters, max_clusters)
...
#WIP: make nice graphs

for (i in 1:length(validation_loglik_list)){
  boxplot(validation_loglik_list[[i]]~num_clusters)
  points(tapply(validation_loglik_list[[i]], num_clusters, mean), col="blue", pch=19)
  legend("bottomleft", inset=.02, legend=c("Mean"), col=c("blue"), pch=19, cex=0.8)
  plot(aic_vector_list[[i]]~c(2:7))
  plot(bic_vector_list[[i]]~c(2:7))
}


```

Let us observe the results using the first 9 variables in the dataset and extracting the true number of clusters ($K=3$). 

```{r}

#WIP: make nice graphs

X = as.matrix(wine[,2:10])
results = EM_algorithm(X = X, K = 3, initialization = "random_centroids")
clusters = classify(X = as.matrix(wine[,2:i]), K = 3, mu = results$mu, sigma = results$sigma, prop = results$prop)

my_cols <- c("#00AFBB", "#E7B800", "#FC4E07")  
pairs(X, pch = 19,  cex = 0.5,
      col = my_cols[clusters],
      lower.panel=NULL)
