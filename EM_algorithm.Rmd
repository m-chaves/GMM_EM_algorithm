...
To start, we will work only with 2 variables: Fixed acidity and Alcohol.  We can see in the following graph that the 3 clusters overlap each other. 
```{r data loading}
data(wine)
X = as.matrix(wine[,c(2,4)])
y = wine[,1]
clp <- clPairs(X, y)
clPairsLegend(0.88, 1.1, class = c("Barolo", "Grignolino", "Barbera"), 
              col = clp$col, pch = clp$pch, title = "Wine data")
```

## Initial clustering

We can plot initial clusters defined by the ellipses with 95% Confidence Interval in the following plot, whose parameters are defined by the mean(centers) and the covariance matrix in each cluster. 
...
X = as.matrix(wine[,c(2,4)])
y = wine[,1]
plt1 <- plot_ellipses_CI(X,K,y) +
  xlab(colnames(X)[1]) +
  ylab(colnames(X)[2]) + scale_color_manual(labels = c("Barolo", "Grignolino", "Barbera"), values = c("#F8766D", "#7CAE00", "#00BFC4"))
plt1
```
...
For each initialization we are going to run the EM algorithm with values of K = 2,3,4,5 clusters so we can compare later and evaluate them with the adjustedRandIndex(ARI) and classError(CE) paramaters. The ARI gives us a measure of the similarity  between the original clustering and the clustering obtained by the EM with GMM, this value is close to 0.0 for any random cluster and is 1.0 when the clusterings are identical. Whereas the CE computes the error rate of EM clustering relative to the original clustering.

Based on these parameters we might select the clustering with better performance as the one with values ARI near 1 and CE near 0.
...
if (K==2 & init_EM=="random_centroids"){
  p1<-p1 + labs(title = "Initialization: Random centroids", subtitle = "K = 2")
} else if (K==2 & init_EM=="k_means"){
  p1<-p1 + labs(title = "Initialization: K-means", subtitle = "K = 2")
} else{
  p1<-p1 + labs(subtitle = paste("K =", as.character(K),sep=""))
}
plots_list[[plt_n]] = p1
...
error_rate_vec =c()
for (cnt in c(1:length(c_error_vec))){
  if (cnt%%2==0){
  error_rate_vec = append(error_rate_vec, c_error_vec[[cnt]]) 
}
}


labels_error <- paste("ARI=", as.character(round(adj_rndI_vec,4)), ", CE=", as.character(round(error_rate_vec,4)),sep="")

plt_final <- ggarrange(plotlist = plots_list, nrow = 4, ncol = 2, labels = labels_error ,legend = "none",   hjust = -0.6, vjust = 3.5, font.label = list(size = 12, color = "orangered", face = "plain", family = NULL)) 

#annotate_figure(plt_final, top = text_grob("Clustering with GMM using EM algorithm in wine data where: x1=Alcohol x2=Fixed Acidity ", color = "black", face = "bold", size = 16), left = text_grob("Metrics to evaluate the clustering ARI = Adjusted Rand Index, CE = Class error", color = "blue", face = "bold", size = 14, rot=90))
...
In general we can say that the the initialization with random centroids have a better performance in clustering the bivariate wine dataset. As it is expected we should get a minimum value for CE when K=3, which is coherent with the results in random centroids initialization, however the minimum CE for k-means is for K=4. 

For larger values values of K the clustering with random centroids starts doing erratic clusterings, while the k-means keep the same pattern. 

| Initialization | K  | ARI  | CE |
| --- | --- | --- | --- |
| Random centroids | 2 | 0.2232730 | 0.3988423 |
| K-means | 2 | 0.1716544 | 0.4887640 |
| Random centroids | 3 | **0.3285687** | **0.2977528** |
| K-means | 3| 0.1476214 | 0.4719101 |
| Random centroids | 4 | 0.3225003 | 0.3314607 |
| K-means | 4 | 0.3214188 | 0.4044944 |
| Random centroids | 5 | 0.3537704 | 0.4325843 |
| K-means | 5 | 0.3176420 | 0.4157303 |

Overall the clustering models the one with better performance is with K=3 and random centroids initialization with ARI = 0.3286 and CE = 0.2978

## Historical mu updates

We can also observe the mu updates in the EM algorithm and how they converge

```{r mu plots, fig.height=12, fig.width=12}
...
The convergence with K-means initialization is smother and faster than the random centroids, which make sense since we have already run the k-means clustering to chooses the initial $\mu_i$, and by consequences its number of iterations in general will also be less than random centroids initialization. 

The value of means are different for each initialization, we can observe in the table below for K=3 that there are more difference in the y component

| Initialization | K  | mu1_x  | mu1_y | mu2_x  | mu2_y | mu3_x |mu3_y |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Random centroids | 3 | 12.86083  | 101.14858 | 12.19020 | 67.62052 | 13.62334 | 72.89933 |
| K-means | 3 | 13.11542 | 72.31235 | 12.81306 | 96.41468 | 12.86201 | 114.76510 |


## Historical log-likelihood updates in the EM algorithmq
...
The random centroid get a higher log-likelihood in all the cases which confirms the observations we had in the historical mu and the values of ARI and CE. With this analysis we can certainly affirm that the algorithm of EM-GMM with random centroids outperform any other method reviewed in this work. In general the random centroids should take more iterations to converge.
...
| EM | Random centroids | 3 | -960.7947 | -978.2945 | 
| EM | K-means | 3 | -972.0563 | -989.5561 |

# EM algorithm vs k-means comparison

```{r, fig.height=8, fig.width=10}
kmeans_cl = kmeans(X, centers=3, nstart = 100, iter.max = 25)
plt4 <- plot_ellipses_CI(X,K,kmeans_cl$cluster) + xlab(colnames(X)[1]) + ylab(colnames(X)[2]) + theme(legend.position = "none") + labs(subtitle = "K-means clustering")

plots_list=list()
plots_list[[1]] <- plt1 + labs(subtitle = "Initial clustering")
plots_list[[2]] <- plt2 + labs(subtitle = "EM random centroids init")
plots_list[[3]] <- plt3 + labs(subtitle = "EM K-means init")
plots_list[[4]] <- plt4


#evaluating the clustering
error_rate_vec1 <- c(0,error_rate_vec[3:4],classError(kmeans_cl$cluster, y)[[2]])
adj_rndI_vec1 <- c(1,adj_rndI_vec[3:4],adjustedRandIndex(kmeans_cl$cluster, y))

labels_error <- paste("ARI=", as.character(round(adj_rndI_vec1,4)), ", CE=", as.character(round(error_rate_vec1,4)),sep="")

plt_final <- ggarrange(plotlist = plots_list, nrow = 2, ncol = 2, common.legend = TRUE, labels = labels_error, hjust = -0.6, vjust = 3.5, font.label = list(size = 12, color = "orangered", face = "plain", family = NULL)) 
...
Considering the ARI and CE we can confirm that the EM algorithm with random centroids 

| Algorithm | Initialization | K  | mu1_x  | mu1_y | mu2_x  | mu2_y | mu3_x |mu3_y | ARI | CE |
| EM | Random centroids | 3 | 12.86083  | 101.14858 | 12.19020 | 67.62052 | 13.62334 | 72.89933 | 0.3286 | 0.2978 |
| EM | K-means | 3 | 13.11542 | 72.31235 | 12.81306 | 96.41468 | 12.86201 | 114.76510 | 0.1476 | 0.4719 |
| K-means | - | 3 | 12.95889 | 117.85556 | 12.82167 | 95.70000 | 13.11186 | 71.07835 | 0.1493 | 0.4888 |
