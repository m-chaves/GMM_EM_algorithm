...
author: "Mariana Chaves and Franz Franco"
...
EM_algorithm = function(X,K,max_it=1000, initialization){
...
classify = function(X,K,mu,sigma){
   "
  ...
  "
  probs = sapply(1:K, FUN = function(i){dnorm(X,mu[i],sigma[i])})
  cluster = sapply(1:n, FUN = function(i){which.max(probs[i,])})
...
crossvalidation.EM = function(X, K, k_fold, max_it=1000, initialization, seed = 32){
  ...
  aics: vector with the AICs obtained in the validation sets
  bics: vector with the BICs obtained in the validation sets
  "
  ...
  for (i in 1:k_fold){
    ...
    #get AIC, BIC  and likelihood on validation set
    ...
    aics = append(aics,metric_results$aic)
    bics = append(bics,metric_results$bic)
  }
  
  return(list(logliks = logliks, aics = aics, bics=bics))
...
# Run cross-validation for different number of clusters 
for (number_of_clusters in 2:10) {
  # run cross-validation
  crossval_results = crossvalidation.EM(X, K = number_of_clusters, k_fold = k_fold, initialization = "k_means", seed = 32)
  # save the results 
  validation_loglik = append(validation_loglik,crossval_results$logliks)
  validation_aic = append(validation_aic,crossval_results$aics)
  validation_bic = append(validation_bic,crossval_results$bics)
  num_clusters = append(num_clusters,rep(number_of_clusters,k_fold))
} 

boxplot(validation_aic~num_clusters)
boxplot(validation_bic~num_clusters)
